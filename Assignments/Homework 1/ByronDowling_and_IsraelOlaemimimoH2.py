...
"""
    ...
    Details: 
        ...
        Try and Except is used to catch any variable casting errors that would lead to a runtime 
        error and prematurely end the program.
"""
    ...
    while(True):
        ...
        if num == 'STOP':
            ...
            if len(numbers) >= 2:
                ...
                print("List must contain two or more numbers.\n")
...
def printResults(nums, vars, mean, variance, stdDev):
    ...
    xiSum = float("{:.2f}".format(xiSum))
    xbarSqrtSum = float("{:.2f}".format(xbarSqrtSum))
